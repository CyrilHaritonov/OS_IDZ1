...
int main(int argc, char *argv[]) {
    ...
    if (mkfifo("fifo_1", 0666) < 0) {
        printf("Failed to create fifo 1\n");
        return 0;
    }
    pid_t pid;
    pid = fork();
    ...
    if (mkfifo("fifo_2", 0666) < 0) {
        printf("Failed to create fifo 2\n");
        return 0;
    }
    pid = fork();
    if (pid == -1) {
    ...
    if (pid == -1) {
        ...
        ssize_t check = read(fd_2_read, buffer, sizeof (buffer));
        close(fd_2_read);
        if (check < 0) {
